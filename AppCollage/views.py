...
def AddCourse(request):
    if request.method == 'POST':
        ...
        data = CorceModel(Course_Name=coursename,Course_Fees=coursefees)
        data.save()
        # messages.info(request, 'New User Added')
        return redirect('Index')

def Login_SignUp_Page(request):
    courses=CorceModel.objects.all()
    ...
    if request.method=='POST':
        ...
        if password==confirm_password:
            if User.objects.filter(username=username).exists():
                ...
                cdata=CorceModel.objects.get(id=course)
                ext_user_data=TechrsModel(Teacher_Address=address,Teacher_Gender=gender,Teacher_Age=age,Teacher_Photo=photo,Teacher=data,Course=cdata)
                ext_user_data.save()
                messages.success(request, 'SuccessFully Registered')
                print('success..')
                return redirect('Login_SignUp_Page')
...
def Add_Student(request):
    if request.method=='POST':
        ...
        course=CorceModel.objects.get(id=select)
        data = StdntsModel(Std_Name=stdname,Std_Address=stdaddress,Std_Age=stdage,Course=course)
        data.save()
        return redirect('Index')
...
def Student_Details(request):
    if not request.user.is_staff:
        return redirect('Login_SignUp_Page')
    student_detail = StdntsModel.objects.all()
...
def Teacher_Details(request):
    if not request.user.is_staff:
        return redirect('Login_SignUp_Page')
    teacher_detail = TechrsModel.objects.all()
    return render(request,'admin/teacherdetails.html',{'teacher':teacher_detail})

def Delete_Teacher(request,id):
    if not request.user.is_staff:
        return redirect('Login_SignUp_Page')
    tchr = TechrsModel.objects.get(id=id)
    tchr.delete()
    return redirect('Teacher_Details')

# @login_required(login url='Sigin_signUp_page')
def Profile(request):
    users=TechrsModel.objects.get(Teacher=request.user)
    context={"user":users}
    return render(request,'user/profile.html',context)


def Edit_Page(request):
    teacher=TechrsModel.objects.get(Teacher=request.user)
    return render(request,"user/editprofile.html",{'edit':teacher})

def Edit_Profile(request):
    if request.method=='POST':
        tdata = TechrsModel.objects.get(Teacher=request.user)
